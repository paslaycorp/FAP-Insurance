...
# Legacy /demo has no Request parameter. Keep the route importable while
# preserving SlowAPI enforcement for handlers that actually accept Request.
...
def _compatible_limit(self, *args, **kwargs):
    ...
    def apply(func):
        if "request" not in inspect.signature(func).parameters and "websocket" not in inspect.signature(func).parameters:
            return func
