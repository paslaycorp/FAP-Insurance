...
class ExchangeError(ValueError):
    """Base exchange validation error."""

class BindingError(ExchangeError):
    """Request/evidence/context binding failure."""

class ReplayError(ExchangeError):
    """Previously accepted attestation."""

class SignatureError(ExchangeError):
    """Unknown or invalid attestation signature."""

class CanonicalizationError(ExchangeError):
...
def canonical_bytes(payload: Mapping[str, Any], *, exclude: frozenset[str] = frozenset()) -> bytes:
    if not isinstance(payload, Mapping):
        raise CanonicalizationError("payload must be a mapping")
...
def _unb64(value: str) -> bytes:
    ...
    try:
        ...
        raise SignatureError("signature encoding is invalid") from exc


def _utc(value: datetime) -> datetime:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ExchangeError("timestamp must be timezone-aware")
...
class AssuranceRequest:
    ...
    def payload(self) -> dict[str, Any]:
        return {"exchange_version": EXCHANGE_VERSION, **{k: getattr(self, k) for k in self.__dataclass_fields__}}

    def verify_digest(self) -> None:
        expected = digest_hex(self.payload(), exclude=frozenset({"request_digest"}))
        if not secrets.compare_digest(expected, self.request_digest):
            raise BindingError("request digest mismatch")
...
class AssuranceAttestation:
    ...
    def unsigned_payload(self) -> dict[str, Any]:
        return {k: v for k, v in self.payload().items() if k not in {"response_digest", "signature"}}

    def verify_response_digest(self) -> None:
        expected = digest_hex(self.unsigned_payload())
        if not secrets.compare_digest(expected, self.response_digest):
            raise BindingError("response digest mismatch")


class AttestationSigner:
    def __init__(self, service_id: str, private_key: Ed25519PrivateKey):
        if not service_id:
            raise ExchangeError("service identity is required")
        self.service_id = service_id
        self._private_key = private_key

    @classmethod
    def generate(cls, service_id: str) -> "AttestationSigner":
        return cls(service_id, Ed25519PrivateKey.generate())
    ...
    def sign(self, payload: Mapping[str, Any]) -> str:
        return _b64(self._private_key.sign(canonical_bytes(payload)))


class ReplayGuard:
    """Thread-safe bounded in-process replay guard."""
    ...
    def accept_once(self, responder_service_id: str, nonce: str, request_digest: str) -> None:
        ...
        with self._lock:
            if key in self._seen:
                raise ReplayError("attestation replay detected")
            if len(self._seen) >= self._max_entries:
                raise ReplayError("replay state capacity exhausted")
            self._seen[key] = None
...
def build_attestation(request: AssuranceRequest, *, artifact_id: str, engine_id: str, engine_version: str, policy_id: str, policy_version: str, oracle_versions: tuple[str, ...] = (), processed_at: datetime, result: str, confidence: float | None, failure_state: str | None, signer: AttestationSigner) -> AssuranceAttestation:
    ...
    signature = signer.sign(unsigned | {"response_digest": response_digest})
...
def verify_attestation(request: AssuranceRequest, attestation: AssuranceAttestation, *, trusted_keys: Mapping[str, Ed25519PublicKey], replay_guard: ReplayGuard, expected_engine_id: str | None = None, expected_engine_version: str | None = None, expected_policy_id: str | None = None, expected_policy_version: str | None = None, expected_evidence_id: str | None = None) -> None:
    request.verify_digest()
    if attestation.request_id != request.request_id or attestation.nonce != request.nonce:
        raise BindingError("request correlation mismatch")
    if not secrets.compare_digest(attestation.request_digest, request.request_digest):
        raise BindingError("request digest binding mismatch")
    if attestation.evidence_id != request.evidence_id or (expected_evidence_id is not None and attestation.evidence_id != expected_evidence_id):
        raise BindingError("evidence binding mismatch")
    if expected_engine_id is not None and attestation.engine_id != expected_engine_id:
        raise BindingError("engine identity mismatch")
    if expected_engine_version is not None and attestation.engine_version != expected_engine_version:
        raise BindingError("engine version mismatch")
    if expected_policy_id is not None and attestation.policy_id != expected_policy_id:
        raise BindingError("policy identity mismatch")
    if expected_policy_version is not None and attestation.policy_version != expected_policy_version:
        raise BindingError("policy version mismatch")
    public_key = trusted_keys.get(attestation.responder_service_id)
    if public_key is None:
        raise SignatureError("unknown responder service identity")
    attestation.verify_response_digest()
    try:
        public_key.verify(_unb64(attestation.signature), canonical_bytes(attestation.unsigned_payload() | {"response_digest": attestation.response_digest}))
    except InvalidSignature as exc:
        raise SignatureError("attestation signature is invalid") from exc
