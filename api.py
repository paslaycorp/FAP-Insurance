...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try: req_id = str(uuid.UUID(supplied_id)) if supplied_id else str(uuid.uuid4())
        except (ValueError, AttributeError): req_id = str(uuid.uuid4())
        set_request_id(req_id); request.state.request_id = req_id; start = datetime.now(timezone.utc)
        try: response = await call_next(request)
        except Exception:
            log.error("http.unhandled", traceback=traceback.format_exc()); raise
        finally:
            duration_ms = (datetime.now(timezone.utc) - start).total_seconds() * 1000
            log.info("http.response", status_code=getattr(locals().get("response"), "status_code", 500), duration_ms=round(duration_ms, 2)); clear_request_id()
...
async def validation_handler(request: Request, exc: RequestValidationError):
    req_id = getattr(request.state, "request_id", str(uuid.uuid4())); first = exc.errors()[0] if exc.errors() else {}; field = ".".join(str(x) for x in first.get("loc", []))
...
def _recommendation(verdict: str, score: float) -> str:
    if verdict == "STRICT": return "Photo provenance verified. Proceed with standard claim processing."
    if verdict == "PROBABLE": return "Photo likely authentic. Recommend standard review with spot-check."
    if verdict == "SUSPICIOUS": return "Anomalies detected. Require claimant interview and secondary documentation."
...
def _build_fap_payload(req: VerifyClaimRequest) -> Dict[str, Any]:
    return {"artifact_id": None, "timestamp_claimed": req.timestamp_claimed, "geo": {"lat": req.lat, "lon": req.lon}, "media_hash": req.media_hash or hashlib.sha256(f"{req.claim_id}:{req.timestamp_claimed.isoformat()}".encode()).hexdigest(), "media_type": "image", "device": {"model": req.device_model, "manufacturer": req.device_manufacturer, "os_version": req.device_os, "enrollment_id": req.enrollment_id}, "weather_reported": None, "witness_ids": req.witness_ids}
...
async def _process_single_claim(req: VerifyClaimRequest, fap_client: FapCoreClient, reality: RealityAnchor, fusion: EvidenceFusionEngine, req_id: str) -> VerifyClaimResponse:
    ...
    try:
        ...
        if dpie_context is None: raise FapCoreUnavailable("EPM assurance context is not installed")
        try:
            fap_result = await fap_client.verify_assured(config.FAP_CORE_URL, payload, claim_id=req.claim_id, evidence_id=envelope.evidence_id, timestamp_claimed=req.timestamp_claimed, purpose=dpie_context.source_purpose, scope=dpie_context.source_scope, jurisdiction=dpie_context.source_jurisdiction, rule_id=dpie_context.rule_id, rule_version=dpie_context.rule_version, authority=dpie_context.rule_authority, consequence=dpie_context.consequence)
        ...
        fusion_result = fusion.evaluate(envelope.observations); envelope.confidence_score = fusion_result.confidence; envelope.verdict = fusion_result.verdict
        dpie_result = None
        if dpie_context is not None: dpie_result = assess_request_context(evidence_id=envelope.evidence_id, verification={"verdict": envelope.verdict}, context=dpie_context)
        ...
        envelope.audit_record_hash = audit_record.record_hash; envelope.fap_core_response = fap_result
        if dpie_result and dpie_result["decision"] in {"DENY", "QUARANTINE"}:
            status_code = 403 if dpie_result["decision"] == "DENY" else 409
            raise HTTPException(status_code=status_code, detail={"error": "DPIE_ASSURANCE_BLOCKED", "decision": dpie_result["decision"], "failure": dpie_result["failure"], "reason": dpie_result["reason"], "transition_id": dpie_result["transition_id"], "evidence_id": envelope.evidence_id, "audit_record_hash": audit_record.record_hash})
...
async def verify_claim(request: Request, req: VerifyClaimRequest, api_key: str = Depends(verify_api_key)):
    set_context(req.dpie_context()); return await _process_single_claim(req, app.state.fap_client, app.state.reality, app.state.fusion, request.state.request_id)
...
async def verify_batch(request: Request, requests: List[VerifyClaimRequest], api_key: str = Depends(verify_api_key)):
    if len(requests) > 10: raise HTTPException(status_code=400, detail="Batch limit is 10 claims per request.")
    results = []
    for req in requests:
        req_id = str(uuid.uuid4()); set_context(req.dpie_context())
        try:
            result = await _process_single_claim(req, app.state.fap_client, app.state.reality, app.state.fusion, req_id); results.append({"claim_id": req.claim_id, "status": "ok", "result": result.model_dump()})
        except HTTPException as exc: results.append({"claim_id": req.claim_id, "status": "blocked", "status_code": exc.status_code, "detail": exc.detail})
        except Exception:
            log.error("verify.batch_error", request_id=req_id, claim_id=req.claim_id, traceback=traceback.format_exc()); results.append({"claim_id": req.claim_id, "status": "error", "detail": "Verification failed."})
        finally: clear_context()
...
async def get_audit_report(request_id: str, api_key: str = Depends(verify_api_key)):
    ...
    if not record: raise HTTPException(status_code=404, detail="Audit record not found.")
...
async def get_claim_history(claim_id: str, api_key: str = Depends(verify_api_key)):
    records = get_by_claim_id(claim_id); return {"claim_id": claim_id, "total_verifications": len(records), "records": [r.to_dict() for r in records]}

@app.get("/audit/integrity")
async def audit_integrity(api_key: str = Depends(verify_api_key)): return get_chain_integrity()
...
async def get_evidence(evidence_id: str, api_key: str = Depends(verify_api_key)):
    ...
    if not record: raise HTTPException(status_code=404, detail="Evidence not found.")
    return record.to_dict()

@app.get("/report/{verification_id}", response_class=HTMLResponse)
async def get_report(verification_id: str, api_key: str = Depends(verify_api_key)): return HTMLResponse(content=f"<h1>Legacy endpoint</h1><p>Use /audit/report/{{request_id}}</p><p>ID: {verification_id}</p>")

@app.get("/pricing")
async def pricing(tier: Optional[str] = None):
    tiers = getattr(config, "TIERS", {})
    if tier and tier in tiers:
        item = tiers[tier]; return {"tier": getattr(item, "name", tier), "price_per_verification": getattr(item, "price_per_verification", 0.0), "monthly_cap": getattr(item, "monthly_cap", 0)}
    return {"tiers": list(tiers.keys())}

@app.get("/demo")
async def demo(): return {"service": "FAP-Insurance", "version": SETTINGS.VERSION, "audit_chain": get_chain_integrity(), "architecture": "FAP → DPIE → Governor → Decision → Immutable Audit"}
