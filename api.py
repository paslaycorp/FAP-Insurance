"""FAP-Insurance API: evidence verification with EPM assurance enforcement."""
import traceback
import uuid
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        request.state.received_at = start
        try:
...
async def fap_core_handler(request: Request, exc: FapCoreUnavailable):
    return JSONResponse(
        ...
        content=_err(
            "SERVICE_UNAVAILABLE",
            "FAP-Core temporarily unavailable.",
            getattr(request.state, "request_id", None),
        ),
    )
...
def _production_assurance_context(
    *,
    req: VerifyClaimRequest,
    evidence_id: str,
    received_at: datetime,
    availability,
) -> RequestAssuranceContext:
    source_purpose = "claim-verification"
    source_scope = "claim"
    source_jurisdiction = "TX"
    target_at = req.downstream_at or received_at
    return RequestAssuranceContext(
        ...
        target_purpose=req.downstream_purpose or source_purpose,
        target_scope=req.downstream_scope or source_scope,
        target_jurisdiction=req.downstream_jurisdiction or source_jurisdiction,
        target_at=target_at,
        rule_id=req.downstream_rule_id or "carrier-default",
        rule_version=req.downstream_rule_version or "1",
        rule_authority=req.downstream_rule_authority or "carrier-authority",
        consequence=req.downstream_consequence,
        preservation_proof=req.preservation_proof,
        evidence_availability=availability,
        require_trusted_availability=True,
        event_time=req.timestamp_claimed,
    )
...
async def _process_single_claim(
    ...
    start = datetime.now(timezone.utc)
    received_at = received_at or start
    try:
        if req.media_hash is None:
            raise HTTPException(
                status_code=422,
                detail={
                    "error": "EPM_AVAILABILITY_REQUIRED",
                    "reason": (
                        "Production verification requires a SHA-256 media_hash so "
                        "FAP can bind trusted evidence availability to a concrete "
                        "evidence reference."
                    ),
                    "request_id": req_id,
                },
        ...
        envelope = EvidenceEnvelope(
            evidence_id=(
                f"FAP-EV-{received_at.strftime('%Y%m%d')}-"
        ...
        availability = ingest_fap_authenticated_request(
            FAPAuthenticatedRequestReceipt(
                ...
                authenticated=authenticated_transport,
            )
        )
        dpie_context = _production_assurance_context(
            req=req,
            evidence_id=envelope.evidence_id,
            received_at=received_at,
            availability=availability,
        )
        set_context(dpie_context)
        ...
        envelope.add_observation(
            OracleObservation(
                ...
                discrepancy_note=(
                    solar_result.reason if solar_result.status != "OK" else None
                ),
            )
        )
        if solar_result.status != "OK":
            record_source_degradation("NOAA_SWPC")

        try:
            weather_data = await reality.check_weather(req.lat, req.lon)
            weather_available = bool(weather_data.get("available"))
            envelope.add_observation(
                OracleObservation(
                    oracle_type="weather",
                    source="Open-Meteo_DIRECT",
                    confidence=0.5 if weather_available else 0.0,
                    raw_value=weather_data,
                    status="OK" if weather_available else "DEGRADED",
                )
            )
            if not weather_available:
                record_source_degradation("Open-Meteo_DIRECT")
        ...
        envelope.add_observation(
            OracleObservation(
                ...
                confidence=0.85 if req.enrollment_id else 0.3,
                status="OK" if req.enrollment_id else "DEGRADED",
                discrepancy_note=None if req.enrollment_id else "Device not enrolled.",
            )
        )
        ...
        try:
            ...
            envelope.add_observation(
                OracleObservation(
                    ...
                    source="FAP-Core",
                    confidence=float(fap_result.get("total_score", 0.0)),
                    raw_value={
            ...
            record_source_degradation("FAP-Core")
            envelope.add_observation(
                OracleObservation(
                    oracle_type="consensus",
                    source="FAP-Core",
        ...
        epm_started = datetime.now(timezone.utc)
        ...
        temporal = assess_temporal_availability(
            evidence_id=envelope.evidence_id,
            state_at=dpie_context.target_at,
            availability=availability,
        )
        record_epm_decision(
            dpie_result,
            latency_ms=epm_latency_ms,
            availability_status=temporal.status.value,
            fap_core_connected=fap_core_connected,
        )

        elapsed_ms = int((datetime.now(timezone.utc) - start).total_seconds() * 1000)
        report_html = AdjusterReport(
        ...
        audit_record = store_verification(
            ...
            components=(
                fap_components
                or {o.oracle_type: o.confidence for o in envelope.observations}
            ),
            request_payload=req.model_dump(),
            envelope=envelope.to_audit_payload(),
            reality_anchor={"solar": solar_result.to_dict()},
            raw_fap_response=fap_result or None,
        ...
        return VerifyClaimResponse(
            claim_id=req.claim_id,
            verification_id=audit_record.evidence_id,
            verdict=envelope.verdict,
            verdict_label=_map_verdict(envelope.verdict),
            score=round(envelope.confidence_score, 4),
            confidence=round(float(fap_result.get("confidence", 0.0)), 4),
            components=fap_components,
            solar_flux_at_time=solar_result.flux,
            weather_match=fap_components.get("weather"),
            device_enrolled=bool(req.enrollment_id),
...
@app.get("/epm/metrics")
async def epm_metrics(api_key: str = Depends(verify_api_key)):
    return epm_metrics_snapshot()
...
async def verify_claim(
    ...
    return await _process_single_claim(
        req,
        app.state.fap_client,
        app.state.reality,
        app.state.fusion,
...
async def verify_batch(
    request: Request,
    requests: List[VerifyClaimRequest],
    api_key: str = Depends(verify_api_key),
):
    if len(requests) > 10:
        raise HTTPException(
            status_code=400,
            detail="Batch limit is 10 claims per request.",
        )
    results = []
    for req in requests:
        req_id = str(uuid.uuid4())
        try:
            result = await _process_single_claim(
                req,
                app.state.fap_client,
                app.state.reality,
                app.state.fusion,
...
async def get_audit_report(request_id: str):
    ...
    return HTMLResponse(
        content=generate_audit_report(record, get_chain_integrity())
    )
...
async def get_report(verification_id: str):
    return HTMLResponse(
        content=(
            ...
            "<p>Use /audit/report/{request_id}</p>"
            f"<p>ID: {verification_id}</p>"
        )
    )
...
async def pricing(tier: Optional[str] = None):
    ...
    if tier and tier in tiers:
        item = tiers[tier]
        return {
            "tier": getattr(item, "name", tier),
            "price_per_verification": getattr(
                item,
                "price_per_verification",
                0.0,
            ),
            "monthly_cap": getattr(item, "monthly_cap", 0),
        }
...
async def demo():
    return {
        ...
        "version": SETTINGS.VERSION,
        "audit_chain": get_chain_integrity(),
        "architecture": "FAP → EPM → Governor → Decision → Immutable Audit",
    }
