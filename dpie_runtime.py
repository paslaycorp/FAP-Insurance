...
from dpie_assurance import AssuranceContext, AssuranceState, FailureCode, PreservationProof, RuleBinding, State, Transition, evaluate_transition
...
def _proof_from_mapping(raw: Optional[Mapping[str, Any]], transition_id: str) -> Optional[PreservationProof]:
    ...
    return PreservationProof(property_name=str(raw.get("property_name", "applicability")), transition_id=str(raw.get("transition_id", transition_id)),
                             ...
                             source_purpose=raw.get("source_purpose"), target_purpose=raw.get("target_purpose"), source_scope=raw.get("source_scope"),
                             target_scope=raw.get("target_scope"), source_jurisdiction=raw.get("source_jurisdiction"), target_jurisdiction=raw.get("target_jurisdiction"))
...
def assess_fap_transition(*, evidence_id: str, verification: Mapping[str, Any], source_context: FAPDecisionContext, target_context: FAPDecisionContext, transition_id: str, preservation_proof: Any = None) -> Mapping[str, Any]:
    ...
    material = any((source_context.purpose != target_context.purpose, source_context.scope != target_context.scope, source_context.jurisdiction != target_context.jurisdiction,
                    source_context.at != target_context.at, source_context.rule_id != target_context.rule_id, source_context.rule_version != target_context.rule_version,
    ...
    return {"transition_id": assurance.transition_id, "property": assurance.property_name, "state": assurance.state.value, "decision": decision.value,
            ...
            "source_evidence_id": evidence_id, "fail_closed": decision in {__import__('dpie_assurance').Decision.DENY, __import__('dpie_assurance').Decision.QUARANTINE}}

def assess_request_context(*, evidence_id: str, verification: Mapping[str, Any], context: RequestAssuranceContext) -> Mapping[str, Any]:
    source = FAPDecisionContext(None, context.source_purpose, context.source_scope, context.source_jurisdiction, context.source_at,
                                "carrier-default", "1", "carrier-authority", context.consequence)
    target = FAPDecisionContext(None, context.target_purpose, context.target_scope, context.target_jurisdiction, context.target_at,
                                context.rule_id, context.rule_version, context.rule_authority, context.consequence)
    return assess_fap_transition(evidence_id=evidence_id, verification=verification, source_context=source, target_context=target,
                                 transition_id=f"DPIE-{evidence_id or 'PENDING'}", preservation_proof=context.preservation_proof)
