...
async def fetch_solar_flux() -> Dict[str, Any]:
    ...
    try:
        response = await _http.get(SETTINGS.NOAA_SWPC_URL, timeout=SETTINGS.HTTP_TIMEOUT_SECONDS); response.raise_for_status()
        data = response.json()
        ...
        _solar_cache[cache_key] = result; return result
    except Exception as exc:
        log.error("solar.fetch_failed", error=str(exc)); raise
...
async def fetch_weather(lat: float, lon: float) -> Dict[str, Any]:
    ...
    try:
        ...
        log.error("weather.openmeteo_failed", error=str(exc), lat=lat, lon=lon); raise RuntimeError(f"All weather oracles failed for ({lat}, {lon}).") from exc

async def _fetch_nws_weather(lat: float, lon: float) -> Dict[str, Any]:
    if _http is None: raise RuntimeError("HTTP client not initialized.")
    points_url = f"{SETTINGS.NOAA_NWS_BASE}/points/{lat:.4f},{lon:.4f}"
    response = await _http.get(points_url, timeout=SETTINGS.HTTP_TIMEOUT_SECONDS); response.raise_for_status()
    forecast_url = response.json()["properties"]["forecast"]
    forecast_response = await _http.get(forecast_url, timeout=SETTINGS.HTTP_TIMEOUT_SECONDS); forecast_response.raise_for_status()
...
async def _fetch_open_meteo_weather(lat: float, lon: float) -> Dict[str, Any]:
    ...
    response = await _http.get(url, timeout=SETTINGS.HTTP_TIMEOUT_SECONDS); response.raise_for_status()
...
class FapCoreClient:
    """Transport adapter for FAP-Core, including Phase-1 bearer authentication."""
    ...
    async def verify(self, base_url: str, payload: Dict[str, Any], assurance_header: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            response = await self.client.post(f"{base_url.rstrip('/')}/verify", json=payload, headers=headers)
